...
export function extractLinks(html, siteUrl) {
  ...
  // Scope to post content area only, so that sidebar/nav/footer links
  // from a live-fetched full page are excluded. Use .h-entry (not .e-content)
  // so that microformat property links like u-in-reply-to, u-like-of,
  // u-repost-of, u-bookmark-of are included — these are rendered outside
  // .e-content but still inside the .h-entry root.
  // Falls back to the whole document for stored content fragments.
  const contentRoot = $(".h-entry").first().length
    ? $(".h-entry").first()
