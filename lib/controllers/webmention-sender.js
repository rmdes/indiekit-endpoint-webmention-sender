  ...
  async post(request, response, next) {
    try {
      ...
      for (const post of posts) {
        ...
        if (!pageHtml) {
          console.log(`[webmention] Skipping ${postUrl} — page not live yet, will retry next cycle`);
          continue;
        }

        // Use page HTML for link extraction; fall back to stored content if available
        const postContent = post.properties.content?.html || post.properties.content?.value || "";
        const contentToProcess = postContent || pageHtml;
